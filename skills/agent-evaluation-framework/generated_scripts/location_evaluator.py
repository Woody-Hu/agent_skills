...
class LocationEvaluator:
    ...
    def _load_config(self, config_file):
        ...
        # 尝试自动查找配置文件
        default_config_path = "config/location_config.json"
    ...
    def _get_task_complexity(self, task):
        """根据任务内容判断复杂度"""
        task = task.lower()
        if any(keyword in task for keyword in ["复杂", "附近", "推荐", "导航", "路线"]):
            return "complex"
        elif any(keyword in task for keyword in ["查询", "获取", "位置"]):
            return "simple"
        else:
            return "medium"
    ...
    def _generate_tools(self):
        ...
        tools = []
        tools.append(MockTool("get_current_location", "获取用户当前位置"))
        tools.append(MockTool("search_nearby", "搜索附近的地点"))
        tools.append(MockTool("calculate_route", "计算路线"))
        tools.append(MockTool("get_location_details", "获取位置详细信息"))
    ...
    def evaluate_skill(self, task: str, iterations: int = None) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            # 模拟执行
            exec_time = random.uniform(0.8, 1.1)
            total_time += exec_time
            model_calls += calls_per_iteration
            time.sleep(0.1)
    ...
    def run_evaluation(self, test_cases):
        ...
        for test_case in test_cases:
            task = test_case.get("input", "获取我的位置")
...
if __name__ == "__main__":
    # 加载配置文件（如果存在）
    config_file = "config/location_config.json"
    evaluator = LocationEvaluator(config_file)
    ...
    test_cases = [
        {
            "name": "简单位置查询",
            "input": "获取我的当前位置",
            "expected_output": "用户当前位置信息"
        },
        {
            "complex位置推荐",
            "input": "推荐我附近的 restaurants",
            "expected_output": "附近餐厅推荐列表"
        }
    ]
    ...
    # 保存结果
    with open("location_evaluation_results.json", "w", encoding="utf-8") as f:
