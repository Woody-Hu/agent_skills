...
class DocumentEvaluator:
    ...
    def _load_config(self, config_file):
        ...
        # 尝试自动查找配置文件
        default_config_path = "config/document_config.json"
    ...
    def _get_task_complexity(self, task):
        """根据任务内容判断复杂度"""
        task = task.lower()
        if any(keyword in task for keyword in ["复杂", "详细", "分析", "提取", "总结"]):
            return "complex"
    ...
    def _generate_tools(self):
        ...
        tools = []
        tools.append(MockTool("load_document", "加载文档内容"))
        tools.append(MockTool("search_document", "搜索文档中的信息"))
        tools.append(MockTool("extract_information", "从文档中提取信息"))
        tools.append(MockTool("summarize_document", "总结文档内容"))
    ...
    def evaluate_mcp(self, task: str, iterations: int = None) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            # 模拟执行
            exec_time = random.uniform(0.9, 1.4)
            total_time += exec_time
            model_calls += calls_per_iteration
            time.sleep(0.1)
    ...
    def evaluate_skill(self, task: str, iterations: int = None) -> Dict[str, Any]:
        ...
        for i in range(iterations):
            # 模拟执行
            exec_time = random.uniform(1.1, 1.6)
            total_time += exec_time
            model_calls += calls_per_iteration
            time.sleep(0.1)
    ...
    def run_evaluation(self, test_cases):
        ...
        for test_case in test_cases:
            task = test_case.get("input", "根据文档回答问题")
...
if __name__ == "__main__":
    # 加载配置文件（如果存在）
    config_file = "config/document_config.json"
    evaluator = DocumentEvaluator(config_file)
    ...
    test_cases = [
        {
            "name": "简单文档查询",
            "input": "查询文档中的关键信息",
            "expected_output": "文档中的关键信息"
        },
        {
            "name": "复杂文档分析",
            "input": "详细分析并总结文档中的主要观点和论证结构",
            "expected_output": "文档的主要观点和论证结构总结"
        }
    ]
    ...
    # 保存结果
    with open("document_evaluation_results.json", "w", encoding="utf-8") as f:
